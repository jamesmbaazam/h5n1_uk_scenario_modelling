...
We implemented a stochastic branching process model to simulate international disease spread between the US (origin) and UK (destination). Each simulation began with a single infected case in the US. The number of secondary cases produced by each infected individual was drawn from a negative binomial distribution with mean equal to the reproduction number (R₀) and dispersion parameter (k). We explored different transmission scenarios with R₀ values of 1.5 and 2.0, and k values of 0.1 (overdispersed) and 1000 (homogeneous). The timing between successive cases followed a gamma distribution fitted to non-zoonotic influenza serial interval data. Each transmission chain evolved independently, with subsequent infections occurring according to these parameters.
...
1. Pre-flight testing only: Passengers were tested before departure with time-dependent test sensitivity. Positive cases were prevented from travelling. Test sensitivity was modeled using a COVID-19 PCR sensitivity curve scaled to match the relative duration of influenza's serial interval. This scaling accounts for the reportedly longer progression of H5N1 compared to COVID-19, while maintaining the biological profile of PCR test sensitivity. The sensitivity function incorporates uncertainty through sampling from the 95% confidence intervals of the scaled curve.
...
Each testing stage incorporated realistic features including time-dependent test sensitivity with uncertainty sampling, delays in receiving results, and imperfect compliance. Implementation delays accounted for the time needed to establish testing infrastructure and train personnel.
